...
defmodule MyBroadway do
  ...
  def start_link(context, index) do
    Broadway.start_link(__MODULE__,
      name: Module.concat([__MODULE__, to_string(index)]),
      context: context,
      producer: [
  ...
  def handle_message(_, message, %{caller_pid: caller_pid}) do
    Process.sleep(2)
    if message.data == "1000" do
      send caller_pid, :start_other_broadway
    end
...
defmodule Runner do
  def run do
    ...
    {broadway_pid, messages_agent} = start_broadway(1)

    other_broadway_pid =
      receive do
        :start_other_broadway ->
          {other_broadway_pid, _messages_agent} = start_broadway(2, messages_agent)
          other_broadway_pid
      end
    # TODO: Find a better way to do this
    receive do
    ...
    stop_broadway(other_broadway_pid)
  ...
  defp start_broadway(index, messages_agent \\ nil) do
    messages_agent =
      if messages_agent do
        messages_agent
      else
    ...
    {:ok, broadway_pid}  = MyBroadway.start_link(context, index)
